...
# echo ""
# echo ""
# # Thay đổi ở đây để làm nổi bật link
# log_and_echo "#####################################################################"
...
# - Xử lý secret và chạy lệnh tương tự phiên bản cũ
# - Kiểm tra kỹ việc tải file proxy-multi.conf
# - Thêm kiểm tra nếu proxy đã chạy thì không cài mới
...
# --- Bước 0: Kiểm tra xem MTProxy đã chạy chưa ---
log_and_echo "[0/9] Kiểm tra xem có tiến trình MTProxy nào đang chạy không..."
# Sử dụng pgrep với pattern cụ thể hơn để tránh false positives
if pgrep -f "mtproto-proxy -H" > /dev/null; then
    log_and_echo "✅ Phát hiện một hoặc nhiều tiến trình MTProxy đang chạy."
    log_and_echo "Script sẽ không thực hiện cài đặt mới để tránh xung đột."
    log_and_echo "Nếu bạn muốn cài đặt một instance mới hoàn toàn, vui lòng dừng các instance MTProxy cũ trước khi chạy lại script này."
    log_and_echo "Hoặc, nếu bạn muốn tạo THÊM một proxy nữa song song, script này hiện tại không hỗ trợ điều đó."
    log_and_echo "=================================================="
    exit 0
else
    log_and_echo "Không tìm thấy tiến trình MTProxy nào đang chạy. Tiếp tục cài đặt mới..."
fi
echo ""


# --- Bước 1: Cập nhật hệ thống và cài đặt các gói cần thiết ---
log_and_echo "[1/9] Đang cập nhật hệ thống và cài đặt các gói phụ thuộc..." # Đã đổi số bước
export DEBIAN_FRONTEND=noninteractive
apt-get update -yqq > /dev/null 2>&1
if [ $? -ne 0 ]; then
    log_and_echo "LỖI: apt-get update thất bại."
...
# --- Bước 2: Tải mã nguồn MTProxy (GetPageSpeed fork) ---
log_and_echo "[2/9] Đang tải mã nguồn MTProxy (GetPageSpeed fork)..." # Đã đổi số bước
REPO_URL="https://github.com/GetPageSpeed/MTProxy"
REPO_DIR="/opt/MTProxy_GetPageSpeed"
if [ -d "$REPO_DIR" ]; then
  # Nếu script tiếp tục sau bước 0 (tức là không có proxy nào chạy)
  # nhưng thư mục cài đặt cũ vẫn còn, có thể người dùng muốn cài đặt lại.
  # Xóa đi để đảm bảo cài đặt sạch.
  log_and_echo "Tìm thấy thư mục cài đặt cũ $REPO_DIR. Sẽ xóa để cài đặt lại."
  rm -rf "$REPO_DIR"
fi
git clone "$REPO_URL" "$REPO_DIR" > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
# --- Bước 3: Biên dịch MTProxy ---
log_and_echo "[3/9] Đang biên dịch MTProxy..." # Đã đổi số bước
cd "$REPO_DIR" || { log_and_echo "LỖI: Không thể cd vào $REPO_DIR"; exit 1; }
make > /dev/null 2>&1
if [ ! -f "objs/bin/mtproto-proxy" ]; then
    log_and_echo "LỖI: Biên dịch MTProxy thất bại."
...
# --- Bước 4: Chuẩn bị file và thư mục thực thi ---
log_and_echo "[4/9] Đang chuẩn bị trong thư mục thực thi..." # Đã đổi số bước
PROXY_EXEC_PATH="${REPO_DIR}/objs/bin/mtproto-proxy"
WORKING_DIR="${REPO_DIR}/objs/bin"
cd "$WORKING_DIR" || { log_and_echo "LỖI: Không thể cd vào $WORKING_DIR"; exit 1; }
log_and_echo "Đang ở thư mục: $(pwd)"
echo ""

# --- Bước 5: Tạo client secret và tải official proxy secret/config ---
log_and_echo "[5/9] Đang tạo client secret và tải official proxy secret/config..." # Đã đổi số bước
NEW_CLIENT_SECRET=$(head -c 16 /dev/urandom | xxd -p -c 16)
...
# --- Bước 6: Tạo port ngẫu nhiên ---
log_and_echo "[6/9] Đang tạo port ngẫu nhiên..." # Đã đổi số bước
RANDOM_PORT=$(shuf -i 10000-65535 -n 1)
log_and_echo "Port ngẫu nhiên mới: $RANDOM_PORT"
echo ""

# --- Bước 7: Mở port trên Firewall (UFW) ---
log_and_echo "[7/9] Đang mở port $RANDOM_PORT trên Firewall (UFW)..." # Đã đổi số bước
if ! command -v ufw > /dev/null; then
    log_and_echo "Cảnh báo: ufw chưa được cài đặt."
else
    if ! ufw status | grep -qw active; then
...
# --- Bước 8: Lấy địa chỉ IP public của máy chủ ---
log_and_echo "[8/9] Đang lấy địa chỉ IP public của máy chủ..." # Đã đổi số bước
SERVER_IP=$(curl -s --max-time 10 ifconfig.me/ip || curl -s --max-time 10 api.ipify.org || hostname -I | awk '{print $1}')
if [ -z "$SERVER_IP" ]; then
    log_and_echo "CẢNH BÁO: Không thể tự động lấy địa chỉ IP. Link có thể không chính xác."
    SERVER_IP="YOUR_SERVER_IP"
...
# --- BƯỚC CUỐI (9/9): TỰ ĐỘNG KHỞI CHẠY PROXY ---
log_and_echo "[9/9] Đang khởi chạy proxy ở chế độ nền..." # Đã đổi số bước
cd "$WORKING_DIR" || exit
# Xóa log cũ trước khi chạy mới
if [ -f "${LOG_PROXY_OUTPUT_FILE}" ]; then
    rm -f "${LOG_PROXY_OUTPUT_FILE}"
